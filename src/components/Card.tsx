...
const Card: React.FC<Project> = ({ name, description, techList, link, imageSrc }): JSX.Element => {
  return (
    <a
    ...
    >
      <img src={`${imageSrc}`} alt="Sample Project" className="w-full h-64 object-cover" />
      <h3 className="mt-3 ml-3 font-bold text-indigo-100 text-xl mb-2">{name}</h3>
        ...
        {techList !== undefined
          ? techList.map((techItem, idx) => (
              <div className="flex flex-col items-center" key={idx}>
                ...
                <span className="mt-2 mx-3 text-indigo-100 text-sm">{techItem.name}</span>
              </div>
            ))
          : null}
      </div>
