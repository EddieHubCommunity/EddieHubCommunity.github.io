...
const App: React.FC=():JSX.Element=> {
  return (
    <>
    <div
  className="min-w-screen min-h-screen bg-gray-900 flex flex-col items-center justify-center px-5 py-5"
>
  ...
  <h3
    className="text-3xl font-semibold text-indigo-100 leading-tight relative z-10"
  >
    We believe Open Source is for everyone, yes you!
  </h3>
  <h2
    className="text-xl font-semibold text-indigo-100 leading-tight relative z-10"
  >
    OPEN SOURCE is NOT about code, it is about people and collaboration
  </h2>

  <div
    className="flex flex-col items-center bg-white lg:rounded m-12 py-8 px-4 lg:px-16 lg:max-w-4xl"
  >
  ...
  <div id="projects">
    ...
    >
      {projects.map(cardItem =>
        <Card description={cardItem.description} name={cardItem.name} techList={cardItem.techList !== undefined ? cardItem.techList: []} imageSrc={ cardItem.imageSrc} link={cardItem.link}  />
        )}
    </div>
  </div>
  ...
  <div className="container my-8 p-5 bg-white rounded-lg">
    ...
    <div
      className="grid grid-cols-1 gap-5 sm:grid-cols-1 md:grid-cols-1 lg:grid-cols-1 xl:grid-cols-2">
      {moderators.map(moderator=> 
        <HorizontalCard badges={moderator.badges} description={moderator.description} github={moderator.github} imageSrc={moderator.imageSrc} name={moderator.name} website={moderator.website} />
        )}
    </div>
  </div>
