...
import { badges as allBadges } from "../data/data";
...
const HorizontalCard: React.FC<Props> = ({imageSrc, badges, description, github, website, name}):JSX.Element => {
    return (
  ...
  <div className="flex flex-inline">
    ...
    <div
      className="lg:w-2/3 md:w-2/3 sm:w-2/3 px-5 pt-5 sm:pt-2 lg:pt-5 rounded-r-lg">
          {badges.map(badge => 
          <>
          
        <Badge title={badge} classes={allBadges.community.classes} />
        </>
            )}
      <h1
        className="font-medium text-lg sm:text-xl md:text-xl lg:text-xl xl:text-xl font-medium pt-2 sm:pt-1 md:pt-1 lg:pt-1">
        {name}
      </h1>
      <p
        className="font-light text-xs sm:text-sm md:text-sm lg:text-sm xl:text-sm py-1 font-normal hidden md:block lg:block break-words">
        {description}
      </p>
